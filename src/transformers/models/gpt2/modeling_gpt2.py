...
class GPT2Attention_origin(nn.Module):
    def __init__(self, config, is_cross_attention=False, layer_idx=None):
        ...
        if 'prefix-tuning' in config.efficient_methods or 'p-tuning-v2' in config.efficient_methods:
            self.prefix_tuning = PrefixTuning(config, self.num_heads, self.embed_dim)
...
class GPT2Attention(GPT2Attention_origin):
...
class GPT2Block(nn.Module):
    def __init__(self, config, layer_idx=None):
        ...
        if self.is_adapter:
            self.self_attn_adapter = Adapter(hidden_size, config)
            self.ffn_adapter = Adapter(hidden_size, config)
    ...
    def forward(
        ...
        if self.is_adapter:
            attn_output = self.self_attn_adapter(attn_output)
        # residual connection
        ...
        if self.is_adapter:
            feed_forward_hidden_states = self.ffn_adapter(feed_forward_hidden_states)
        # residual connection
...
class GPT2Model(GPT2Model_origin):
    def forward(
        self,
        input_ids=None,
