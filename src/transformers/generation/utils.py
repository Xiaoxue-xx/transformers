...
class GenerationMixin(GenerationMixin_origin):
    """
    ...
    def generate(
        ...
        **model_kwargs,
        ...
        stopping_criteria = self._get_stopping_criteria(max_length=max_length, max_time=max_time, stopping_criteria=stopping_criteria)
        ...
        if is_greedy_gen_mode:
            ...
            # greedy search
            generated_corpus = self.greedy_search(
                input_ids,
                logits_processor=logits_processor,
                stopping_criteria=stopping_criteria,
                pad_token_id=pad_token_id,
            ...
            input_ids, model_kwargs = self._expand_inputs_for_generation(
                input_ids=input_ids, expand_size=num_beams, is_encoder_decoder=self.config.is_encoder_decoder, **model_kwargs
            )
            generated_corpus = self.beam_search(
                input_ids,
                beam_scorer,
                logits_processor=logits_processor,
                stopping_criteria=stopping_criteria,
            ...
            generated_corpus = self.beam_sample(
                input_ids,
                beam_scorer,
                logits_processor=logits_processor,
                logits_warper=logits_warper,
            ...
            generated_corpus = self.group_beam_search(
                input_ids,
                beam_scorer,
                logits_processor=logits_processor,
                stopping_criteria=stopping_criteria,
            ...
            # 12. run beam search
            generated_corpus = self.constrained_beam_search(
                input_ids,
                constrained_beam_scorer=constrained_beam_scorer,
                logits_processor=logits_processor,
                stopping_criteria=stopping_criteria,
        ...
        if is_beam_gen_mode:
            self._update_beam_size(1)

        assert generated_corpus is not None
...
class BeamSearchScorerV2(BeamSearchScorer):
    ...
    def process(
        ...
        if (beam_indices is not None):
            warnings.warn("Fastseq do not support 'beam_indices' and it will be ignored.")
        ...
        for i in range (0, eos_effective_idx_cpu.size()[-1]):
            ...
            batch_idx = torch.div(eos_effective_idx_cpu[i], self.num_beams, rounding_mode='floor')
